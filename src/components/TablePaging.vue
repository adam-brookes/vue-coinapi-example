...
<!--This has been created as a copy of the md-table-pagination component as the current version is broken
    I have created an issue on github here: https://github.com/vuematerial/vue-material/issues/2363
-->
<script lang="babel">
...
export default {
  ...
  watch: {
    ...
    currentPageSize: {
      ...
      handler (newValue, oldValue) {
        if (this.mdUpdate(this.mdPage, this.currentPageSize) !== false) {
          this.updatePage()
        }
      }
    }
  ...
  methods: {
    ...
    updatePage () {
      if (this.currentPageSize !== 0) {
        if (this.isExternalPagination) {
          ...
          if (this.mdPage > 0) {
            const jeff = getPageData(this.mdData, this.mdPage, this.currentPageSize);
            this.$emit('update:mdPaginatedData', jeff);
          } else {
    ...
    changePage (AddOrSubtract) {
      if (this.mdUpdate(this.mdPage + AddOrSubtract, this.currentPageSize) !== false) {
