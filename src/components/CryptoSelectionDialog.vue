  ...
  <div>
    <md-dialog :md-fullscreen="true" :md-active="showDialog">
      <md-dialog-content>
        ...
        <crypto-selection-table v-if="selectedCrypto.crypto === null" class="table-padding-top"
                                @crypto-selected="cryptoSelected"
                                :assets="assets"></crypto-selection-table>
        <md-divider></md-divider>

        <form v-if="selectedCrypto.crypto !== null" novalidate class="md-layout" @submit.prevent="validate">

          <div class="md-layout-item md-large-size-100">
            <md-field>
              <label for="crypto">Crypto Coin</label>
              <md-input :disabled="true" name="crypto" id="crypto" autocomplete="crypto"
...
export default {
  ...
  watch: {
    showDialog(newVal: boolean, oldVal: boolean) {
      if (newVal === true) {
        if (this.editingSelectedCrypto) {
          debugger;
          this.selectedCrypto.update(this.editingSelectedCrypto);
          this.form.priceBought = this.selectedCrypto.priceBought;
          this.form.quantity = this.selectedCrypto.quantity;
          this.form.dateBought = new Date(this.selectedCrypto.dateBought);
        }
      } else {
        this.selectedCrypto = new SelectedCrypto();
        this.form = {
          quantity: null,
          priceBought: null,
          dateBought: null
